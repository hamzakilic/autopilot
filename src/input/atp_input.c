...
void atp_input_log(void *ptr){
	...
	while(input_data->work){
		...
		buffer[5]=input_data->gps_location.sealevel>>24;
		buffer[6]=input_data->gps_location.sealevel>>16;
		buffer[7]=input_data->gps_location.sealevel>>8;
		buffer[8]=input_data->gps_location.sealevel;

		buffer[9]=input_data->gps_location_ex.gps_fix;
		buffer[10]=input_data->gps_location_ex.status;

		buffer[11]=input_data->gps_location_ex.msss>>24;
		buffer[12]=input_data->gps_location_ex.msss>>16;
		buffer[13]=input_data->gps_location_ex.msss>>8;
		buffer[14]=input_data->gps_location_ex.msss;

		buffer[15]=input_data->gps_time.year>>8;
		buffer[16]=input_data->gps_time.year;

		buffer[17]=input_data->gps_time.month;
		buffer[18]=input_data->gps_time.day;
		buffer[19]=input_data->gps_time.hour;
		buffer[20]=input_data->gps_time.min;
		buffer[21]=input_data->gps_time.sec;

		buffer[22]=input_data->motor.values[0]>>8;
		buffer[23]=input_data->motor.values[0];

		buffer[24]=input_data->motor.values[1]>>8;
		buffer[25]=input_data->motor.values[1];

		buffer[26]=input_data->motor.values[2]>>8;
		buffer[27]=input_data->motor.values[2];

		buffer[28]=input_data->motor.values[3]>>8;
		buffer[29]=input_data->motor.values[3];
...
em_uint32 atp_input_create(atp_input **address){
    ...
    //start logging
    input_data->work=1;
    atp_thread_create(&input_data->thread_id,atp_input_log,input_data);
